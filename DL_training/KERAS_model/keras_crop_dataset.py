    ...
    for file in glob.glob(PATH_DATA + dirname + "/*.jpg"):
        ...
        while True:
            ...
            if contours.__len__() == 0:
                ...
                tl_x = max(0, int(xx - expansion_param*w))
                tl_y = max(0, int(yy - expansion_param*h))
                br_x = min(image.shape[1], int(xx + w + expansion_param*w))
                br_y = min(image.shape[0], int(yy + h + expansion_param*h))
